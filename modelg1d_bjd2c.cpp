...
void next_iteration( int t /* in computational units */ )
{
  ...
  if( !tdiv.rem )
  {
	...
	// open file and allow append to file
	std::ofstream myfile( "/home/bjd/software/SQK_2/crank_nicolson/model_g_1d-master/modelg_1d_outfile_3.txt", std::ios::app );
  	myfile << tdiv.quot << ' ' << pnext[G][Mx/2] << endl;
  	myfile << tdiv.quot << ' ' << pnext[X][Mx/2] << endl;
  	myfile << tdiv.quot << ' ' << pnext[Y][Mx/2] << endl;
  	myfile << tdiv.quot << ' ' << lapG << endl;
	myfile << tdiv.quot << ' ' << lapX << endl;
	myfile << tdiv.quot << ' ' << lapY << endl;
